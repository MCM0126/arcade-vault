...
function MiniCard({ id, cover, title, cat }: { id: string; cover: string; title: string; cat: string }) {
  return (
    <Link href={`/games/${id}`} className="mini-card">
      <div className="mini-cover"><div className={`cover-bg ${cover}`} /></div>
      ...
      <section className="home-section reveal">
        ...
        <div className="mini-rail">
          {GAMES.slice(0, 6).map((g) => (
            <MiniCard key={g.id} id={g.id} cover={g.cover} title={g.title} cat={g.cat} />
          ))}
        </div>
